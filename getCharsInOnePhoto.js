...
let imagePath = imageDir + 'big_ccp_ALL18_nodiff_nzd.png';
    ...
    Jimp.read(imagePath)
        .then(function (image) {
            ...
            for (let i = 0; i < allColorLump.length; i++) {
                ...
                rotateImage30Degree(allColorLump[i], 3, 'char' + i + '.jpg')
            }
...
let visitPixelDown = function (pixel, pixelsVisited, allpixelsInColorLump, image) {
    let pixelToVisit = getDownPixel(pixel, image);
    visitPixelBeside(pixelToVisit, pixelsVisited, allpixelsInColorLump, image)
};
...
let rotateImage30Degree = Async(function (colorLump, step, lumpName) {
    ...
    let validImage = Await(getImageSync(validImagePath));
    validImage.write(imagePath);
